...
const handleEndorse = async () => {
  if (!Object.is(NaN, endorsement.value)) {
    if (endorsed.value) {
      ...
      mutate({ endorsement: newEndorsement }).then(({ data }) => {
...
const formatedEndorse = (endorsement) => {
  return Intl.NumberFormat('en', { notation: 'compact' }).format(parseInt(endorsement).toExponential())
}
</script>
<template>
  <div
    class="absolute xl:bottom-[36%] md:bottom-[24%] sm:bottom-[16%] bottom-0 sm:right-6 right-4  flex sm:gap-8 gap-6 items-center">
    <p class="sm:text-sm text-xs font-semibold text-primary">{{ formatedEndorse(endorsement) }}
      endorsements</p>
    <button type="button" @click="handleEndorse" :disabled="loading"
